"""Static ticker -> (company name, sector) map (port of lib/sectorMap.ts)."""
...
def static_profile(ticker: str) -> dict | None:
    """Returns {"name", "sector"} for well-known tickers, else None."""
    hit = _RAW.get(ticker.strip().upper())
    return {"name": hit[0], "sector": hit[1]} if hit else None
